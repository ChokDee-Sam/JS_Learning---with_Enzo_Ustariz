...
function playAShuffledSong() {
    const musicsWithoutCurrentSong = musicsData.filter(
        (el) => el.id !== currentMusicIndex
    ); // toutes les musiques sauf celle en cours
    const randomMusic =
        musicsWithoutCurrentSong[
            Math.trunc(Math.random() * musicsWithoutCurrentSong.length)
        ]; // musique au hasard
